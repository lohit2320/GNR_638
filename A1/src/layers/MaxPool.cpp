...
Tensor<double> &MaxPool::forward(Tensor<double> &input) {
    ...
    output_ = Tensor<double>(4, out_dims);
                ...
                for (int w = 0; w < W_out; ++w) {
                    ...
                    for (int i = 0; i < size_; ++i) {
                        for (int j = 0; j < size_; ++j) {
                            ...
                            // Boundary check
                            if (cur_h < H && cur_w < W) {
                                double val = input.get(n, c, cur_h, cur_w);
                                if (val > max_val) {
                                    max_val = val;
...
Tensor<double> MaxPool::backprop(Tensor<double> chainGradient, double learning_rate) {
    ...
    Tensor<double> grad_input(input_.num_dims, input_.dims);
    grad_input.zero();
        ...
        for (int c = 0; c < C; ++c) {
            for (int h = 0; h < H_out; ++h) {
                for (int w = 0; w < W_out; ++w) {
                    ...
                    double grad = chainGradient.get(n, c, h, w);
                    ...
                    if (max_idx != -1) {
                        ...
                        double current_val = grad_input.get(n, c, h_in, w_in);
