...
def _scatter_plot(proj, labels, title, save_path):
    ...
    sc = ax.scatter(proj[:, 0], proj[:, 1],
                    c=labels, cmap='tab20', alpha=0.7, s=18, linewidths=0)
    cbar = plt.colorbar(sc, ax=ax, ticks=range(NUM_CLASSES))
...
def plot_embeddings(model_name, features, labels):
    ...
    proj = tsne.fit_transform(feats_pre)

    _scatter_plot(
        proj,
        labels,
        f't-SNE – {model_name}',
        os.path.join(PLOT_DIR, f'tsne_{model_name}.png')
    )
    print(f"  [✓] t-SNE saved for {model_name}.")
    ...
    proj = reducer.fit_transform(feats_pre)   # use reduced features

    _scatter_plot(
        proj,
        labels,
        f'UMAP – {model_name}',
        os.path.join(PLOT_DIR, f'umap_{model_name}.png')
    )
    print(f"  [✓] UMAP saved for {model_name}.")
